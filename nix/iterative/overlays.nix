{
  ...
  iteractive = final: prev: {
    ...
    iterative-telemetry = prev.python3Packages.callPackage ./packages/iterative-telemetry.nix {};
    python3 =
      ...
      (
        old: {
          packageOverrides = prev.lib.composeExtensions (old.packageOverrides or (_: _: {})) (selfPythonPackages: pythonPackages: let
            fetchDvc = name: inputs:
              pythonPackages."${name}".overridePythonAttrs (old: {
                inherit
                  (final.iteractive-sources."${name}")
                  src
                  pname
                  version
                  ;
          ...
          in {
            ...
            dvclive = (fetchDvc "dvclive" [pythonPackages.ruamel_yaml]).overridePythonAttrs (old: {
              postPatch = ''
                rm -rf setup.cfg
              '';
            });
    ...
    dvc = prev.dvc.overridePythonAttrs (old: let
      format = prev.formats.toml {};

      pyproject = format.generate "pyproject.toml" readPyproject;
      readPyproject =
        prev.lib.recursiveUpdate (builtins.fromTOML (builtins.readFile (final.iteractive-sources.dvc.src + "/pyproject.toml")))
        {
          project.description = ''
    ...
    in {
      format = "pyproject";
      inherit
        (final.iteractive-sources.dvc-latest)
        src
        pname
        version
        ;
